<template>
  <div class="balance-card">
    ...
    <p class="balance-amount">{{ formatted }}</p>
    <slot />
  </div>
</template>
...
.balance-card::before {
  ...
  background: rgba(249, 107, 238, 0.15); pointer-events: none;
}
.balance-label { font-size: 12px; letter-spacing: 0.8px; opacity: 0.65; text-transform: uppercase; }
.balance-amount { font-size: 2.75rem; font-weight: 300; letter-spacing: -1.4px; margin: 8px 0 4px; }
.balance-meta { font-size: 13px; opacity: 0.6; }
.balance-actions { display: flex; gap: 10px; margin-top: 1.5rem; position: relative; z-index: 1; }
.bal-btn {
